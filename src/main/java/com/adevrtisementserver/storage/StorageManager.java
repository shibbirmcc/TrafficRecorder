...
public class StorageManager {
    ...
    public void init() throws FileNotFoundException{
    	...
    	if(new File(queuefileName).exists()) {
    		...
    		// TODO: hasNext is not enough, need to handle EOF perfectly inside the while loop, 
	...
	public synchronized void saveToStorage(Traffic t){
		try{
			...
			FileChannel.open(Paths.get(queuefileName), StandardOpenOption.APPEND).write(ByteBuffer.wrap(jsonLine.getBytes(StandardCharsets.UTF_8)));
		}catch(Exception e){
			logger.error("", e);
		}
	}
